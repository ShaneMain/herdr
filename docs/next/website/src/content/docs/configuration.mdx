...
`agent_panel_sort` can be `natural`, `working-first`, or `attention`. The default `natural` lists agents in workspace order. Use `working-first` to list agents that are currently working at the top of the panel; other agents keep their workspace order below them. Use `attention` to order agents by how much they need you: blocked first, then done, working, idle, and unknown. Within the same status, the most recently changed agent comes first so a large group surfaces what just transitioned, and agents that have not changed since startup keep their workspace order.
